...
$.ajax({
    ...
    success: function (result) {
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            '<div class="photoCard">' +
            '<img src="' + photoLink + '" alt="Profile photo" />' +
            '<div class="textInfo">' +
                '<p>' + CSVdata[i].Nome + '</p>' +
                '<p class="category">(' + CSVdata[i].Categoria + ')</p>' +
                '<div class="moreText">' +
                    '<br />' +
                    '<p class="advisor"><strong>Orientador:</strong> ' + CSVdata[i].Orientador + '</p>' +
                    '<div class="buttons">' +
                        '<a href="' + CSVdata[i].Lattes + '" target="_blank"><i class="ai ai-lattes"></i> Lattes</a>' +
                        '<a href="' + CSVdata[i].Scholar + '" target="_blank"><i class="ai ai-google-scholar"></i> Google Scholar</a>' +
                    '</div></div></div></div>';
                }
        }
    }
